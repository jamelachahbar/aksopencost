...
# Update OpenCost Helm release to include cloud costs configuration
resource "helm_release" "opencost_cloud_costs" {
  count = var.enable_cloud_costs ? 1 : 0
  ...
  # This will update the existing release
  replace = true
  ...
  set {
    name  = "opencost.exporter.cloudProviderApiKey"
    value = ""
  }

  set {
    name  = "opencost.exporter.azureStorageSecretName"
    value = "azure-storage-config"
  }
  ...
  depends_on = [
    ...
    azurerm_cost_management_export_resource_group.opencost
  ]
}
